...
export default function RequesterStaticMap({ 
  ...
  useEffect(() => {
    ...
    const loadMapTiles = async () => {
      try {
        // Create temporary image for map tiles
        const mapImage = new Image();
        const tileUrl = `https://tile.openstreetmap.org/${zoom}/${x}/${y}.png`;
        
        mapImage.crossOrigin = 'anonymous';
        mapImage.onload = () => {
          // Draw map tile as background
          ctx.drawImage(mapImage, 0, 0, width, height);
          
          // Add subtle overlay for better contrast
          const overlay = ctx.createLinearGradient(0, 0, width, height);
          overlay.addColorStop(0, 'rgba(6, 182, 212, 0.05)');
          overlay.addColorStop(1, 'rgba(139, 92, 246, 0.05)');
        ...
        mapImage.onerror = () => {
          // Fallback to gradient background if tile fails
          drawFallbackBackground();
          drawMapElements();
        };
        
        mapImage.src = tileUrl;
      } catch (error) {
        console.warn('Map tile loading failed:', error);
        drawFallbackBackground();
        drawMapElements();
      }
    };
  ...
  const getGoogleMapsUrl = () => {
    const lat = requesterLocation?.lat || 40.7128;
    const lon = requesterLocation?.lon || -74.0060;
      ...
      {showModal && (
        ...
        >
          ...
          >
            ...
            >
              <div className="d-flex align-items-center gap-2">
                <Navigation size={16} color="#F43F5E" />
            ...
            {/* Modal Footer */}
            <div 
              className="px-3 py-3 d-flex gap-2"
              style={{ 
                background: 'rgba(15, 23, 42, 0.5)',
                borderTop: '1px solid rgba(255,255,255,0.05)'
              }}
            >
              <button
                ...
                className="btn btn-gradient d-flex align-items-center gap-2 flex-fill"
              >
                <Navigation size={16} />
                Open in Google Maps
              </button>
